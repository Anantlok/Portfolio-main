  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      particles.forEach(p => {
        ...
        // Updated particle color to match the new 'text-primary' color (#7A9C57)
...
const Window = ({ title, children, className }) => {
  ...
  // --- Updated with new semantic color classes ---
  return (
    <div className={`bg-surface border border-text-primary rounded-lg shadow-xl shadow-text-primary/20 flex flex-col max-h-full ${className}`}>
      ...
      <div className="bg-background p-6 md:p-10 flex-1 overflow-y-auto custom-scrollbar">
        {children}
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div className="relative h-screen font-mixed text-text-primary isolate bg-background">
